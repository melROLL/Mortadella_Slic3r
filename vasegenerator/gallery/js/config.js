 ...
 * vase generator.
 *
 * It should help the guys from wamungo.com to customize the vase generator in 
...
if( !_VASE_CONFIG || typeof _VASE_CONFIG === "undefined" )
    var _VASE_CONFIG = {};

if( !_VASE_CONFIG.IMAGES || typeof _VASE_CONFIG.IMAGES === "undefined" )
    _VASE_CONFIG.IMAGES = {};


// Global constants (modifify when resizing the HTML5 canvas)
_VASE_CONFIG.BEZIER_CANVAS_WIDTH          = 400; // 384; // 512;
_VASE_CONFIG.BEZIER_CANVAS_HEIGHT         = 600; // 512; // 768;

_VASE_CONFIG.PREVIEW_CANVAS_WIDTH         = 400, // 384; // 512;
_VASE_CONFIG.PREVIEW_CANVAS_HEIGHT        = 600; // 512; // 768

/**
 * If set to true the initialisation will resize the canvas elements
 * so they fit into the available screen dimensions.
 **/
_VASE_CONFIG.AUTO_RESIZE_ON_DOCUMENT_LOAD = true;
 ...
 **/
//_VASE_CONFIG.HIDE_PUBLISH_MESH_MENU       = !isVaseGeneratorDomain();  
_VASE_CONFIG.HIDE_PUBLISH_MESH_MENU       = false; 
 ...
 * Valid values: true|false
 **/
_VASE_CONFIG.HIDE_EXPORT_MESH_MENU       = false;  
//_VASE_CONFIG.HIDE_EXPORT_MESH_MENU       = true; 
 ...
 * Valid values: true|false
 **/
//_VASE_CONFIG.HIDE_PRINT_MENU             = false; // true|false
_VASE_CONFIG.HIDE_PRINT_MENU             = !isVaseGeneratorDomain(); 

/**
 * Set the Print->Order_Print sub menu action to the specific javascript action (string).
 *
 * This only takes effect if the _VASE_CONFIG.HIDE_PRINT_MENU is set to false.
 **/
//_VASE_CONFIG.ORDER_PRINT_ACTION          = "_order_send_to_server('store_custom_vase.php?bend=%bend%&bezier_path=%bezier_path%&id=%id%');";
_VASE_CONFIG.ORDER_PRINT_ACTION          = "order_print();";
 ...
 **/
_VASE_CONFIG.PUBLISHING_URL              = "gallery/store_custom_vase.php"; // Relative path


/**
 * Defines the bezier canvas background image to be drawn (string: url).
 **/
_VASE_CONFIG.IMAGES.BEZIER_BACKGROUND    = "img/bg_bezier.png";
 ...
 **/
_VASE_CONFIG.IMAGES.PREVIEW_BACKGROUND   = "img/bg_preview.png";


/**
 * If the autosave function has stored a vase design before in the session cookie
 * and this is set to true, then the stored bezier curve will be loaded (on page load)
 * instead of the default bezier curve.
 **/
_VASE_CONFIG.AUTOLOAD_ENABLED            = true;
